# Saxony (Sachsen) — State Questions (301–310)
[⬅ Back to Main README](README.md)

---

### Question 301 — Coat of Arms
**🇩🇪** Welches Wappen gehört zum Bundesland Sachsen?
**🇬🇧** Which coat of arms belongs to Saxony (Sachsen)?

![Question 301](../images/301-sachsen.png)

| | Deutsch | English |
|---|---|---|
| ○ | Bild 1 | Image 1 |
| ○ | Bild 2 | Image 2 |
| ○ | Bild 3 | Image 3 |
| ✅ | **Bild 4** | **Image 4** |

> **📝 Explanation:** Saxony's coat of arms features a black-and-gold striped shield (the Rautenkranz), a symbol tracing back to the historic Wettin dynasty.

### Question 302 — District
**🇩🇪** Welches ist ein Landkreis in Sachsen?
**🇬🇧** Which of these is a district (Landkreis) in Saxony (Sachsen)?

| | Deutsch | English |
|---|---|---|
| ✅ | **Vogtlandkreis** | **Vogtlandkreis** |
| ○ | Altötting | Altötting |
| ○ | Uckermark | Uckermark |
| ○ | Nordfriesland | North Frisia |

> **📝 Explanation:** Vogtlandkreis is located in Saxony (Sachsen); the other options are districts in different German states.

### Question 303 — Landtag Term
**🇩🇪** Für wie viele Jahre wird der Landtag in Sachsen gewählt?
**🇬🇧** For how many years is the state parliament in Saxony (Sachsen) elected?

| | Deutsch | English |
|---|---|---|
| ○ | 3 | 3 |
| ○ | 4 | 4 |
| ✅ | **5** | **5** |
| ○ | 6 | 6 |

> **📝 Explanation:** The state parliament of Saxony (Sachsen) is elected for a term of 5 years.

### Question 304 — Voting Age
**🇩🇪** Ab welchem Alter darf man in Sachsen bei Kommunalwahlen wählen?
**🇬🇧** From what age can people vote in local elections in Saxony (Sachsen)?

| | Deutsch | English |
|---|---|---|
| ○ | 14 | 14 |
| ○ | 16 | 16 |
| ✅ | **18** | **18** |
| ○ | 20 | 20 |

> **📝 Explanation:** In Saxony (Sachsen), the minimum voting age for local elections is 18.

### Question 305 — State Flag
**🇩🇪** Welche Farben hat die Landesflagge von Sachsen?
**🇬🇧** What colors does the state flag of Saxony (Sachsen) have?

| | Deutsch | English |
|---|---|---|
| ○ | blau-weiß-rot | blue-white-red |
| ✅ | **weiß-grün** | **white-green** |
| ○ | grün-weiß-rot | green-white-red |
| ○ | schwarz-gelb | black-yellow |

> **📝 Explanation:** The state flag of Saxony (Sachsen) is white-green.

### Question 306 — Civic Education
**🇩🇪** Wo können Sie sich in Sachsen über politische Themen informieren?
**🇬🇧** Where can you get information about political topics in Saxony (Sachsen)?

| | Deutsch | English |
|---|---|---|
| ✅ | **bei der Landeszentrale für politische Bildung** | **at the State Agency for Civic Education** |
| ○ | beim Ordnungsamt der Gemeinde | at the municipal public order office |
| ○ | bei den Kirchen | at the churches |
| ○ | bei der Verbraucherzentrale | at the Consumer Advice Centre |

> **📝 Explanation:** Civic/political education information in Saxony (Sachsen) is provided at the State Agency for Civic Education.

### Question 307 — Capital City
**🇩🇪** Die Landeshauptstadt von Sachsen heißt …
**🇬🇧** The state capital of Saxony (Sachsen) is called …

| | Deutsch | English |
|---|---|---|
| ○ | Leipzig. | Leipzig. |
| ○ | Chemnitz. | Chemnitz. |
| ✅ | **Dresden.** | **Dresden.** |
| ○ | Görlitz. | Görlitz. |

> **📝 Explanation:** Dresden. is the state capital of Saxony (Sachsen).

### Question 308 — Map Location
**🇩🇪** Welches Bundesland ist Sachsen?
**🇬🇧** Which (highlighted) federal state is Saxony (Sachsen)?

![Question 308](../images/308-sachsen.png)

| | Deutsch | English |
|---|---|---|
| ○ | Bild 1 | Image 1 |
| ○ | Bild 2 | Image 2 |
| ○ | Bild 3 | Image 3 |
| ✅ | **Bild 4** | **Image 4** |

> **📝 Explanation:** Saxony lies in eastern Germany, bordering Poland and the Czech Republic.

### Question 309 — Head of Government
**🇩🇪** Wie nennt man die Regierungschefin/den Regierungschef in Sachsen?
**🇬🇧** What is the head of government in Saxony (Sachsen) called?

| | Deutsch | English |
|---|---|---|
| ○ | Erste Ministerin/Erster Minister | First Minister |
| ○ | Premierministerin/Premierminister | Prime Minister |
| ○ | Bürgermeisterin/Bürgermeister | Mayor |
| ✅ | **Ministerpräsidentin/Ministerpräsident** | **Minister-President** |

> **📝 Explanation:** The head of government in Saxony (Sachsen) holds the title Minister-President.

### Question 310 — Ministries
**🇩🇪** Welche Ministerin/welchen Minister hat Sachsen nicht?
**🇬🇧** Which minister does Saxony (Sachsen) NOT have?

| | Deutsch | English |
|---|---|---|
| ○ | Justizministerin/Justizminister | Minister of Justice |
| ✅ | **Außenministerin/Außenminister** | **Minister of Foreign Affairs** |
| ○ | Finanzministerin/Finanzminister | Minister of Finance |
| ○ | Innenministerin/Innenminister | Minister of the Interior |

> **📝 Explanation:** Foreign affairs are a federal responsibility in Germany, not a state matter — so no German state has this ministry.

[⬅ Back to Main README](README.md)