# Brandenburg — State Questions (301–310)
[⬅ Back to Main README](README.md)

---

### Question 301 — Coat of Arms
**🇩🇪** Welches Wappen gehört zum Bundesland Brandenburg?
**🇬🇧** Which coat of arms belongs to Brandenburg?

![Question 301](../images/301-brandenburg.png)

| | Deutsch | English |
|---|---|---|
| ○ | Bild 1 | Image 1 |
| ○ | Bild 2 | Image 2 |
| ○ | Bild 3 | Image 3 |
| ✅ | **Bild 4** | **Image 4** |

> **📝 Explanation:** Brandenburg's coat of arms shows a red eagle on a white/silver shield, symbolizing the historic Margraviate of Brandenburg.

### Question 302 — District
**🇩🇪** Welches ist ein Landkreis in Brandenburg?
**🇬🇧** Which of these is a district (Landkreis) in Brandenburg?

| | Deutsch | English |
|---|---|---|
| ✅ | **Prignitz** | **Prignitz** |
| ○ | Rhein-Sieg-Kreis | Rhein-Sieg-Kreis |
| ○ | Vogtlandkreis | Vogtlandkreis |
| ○ | Amberg-Sulzbach | Amberg-Sulzbach |

> **📝 Explanation:** Prignitz is located in Brandenburg; the other options are districts in different German states.

### Question 303 — Landtag Term
**🇩🇪** Für wie viele Jahre wird der Landtag in Brandenburg gewählt?
**🇬🇧** For how many years is the state parliament in Brandenburg elected?

| | Deutsch | English |
|---|---|---|
| ○ | 3 | 3 |
| ○ | 4 | 4 |
| ✅ | **5** | **5** |
| ○ | 6 | 6 |

> **📝 Explanation:** The state parliament of Brandenburg is elected for a term of 5 years.

### Question 304 — Voting Age
**🇩🇪** Ab welchem Alter darf man in Brandenburg bei Kommunalwahlen wählen?
**🇬🇧** From what age can people vote in local elections in Brandenburg?

| | Deutsch | English |
|---|---|---|
| ○ | 14 | 14 |
| ✅ | **16** | **16** |
| ○ | 18 | 18 |
| ○ | 20 | 20 |

> **📝 Explanation:** In Brandenburg, the minimum voting age for local elections is 16.

### Question 305 — State Flag
**🇩🇪** Welche Farben hat die Landesflagge von Brandenburg?
**🇬🇧** What colors does the state flag of Brandenburg have?

| | Deutsch | English |
|---|---|---|
| ○ | blau-weiß-rot | blue-white-red |
| ✅ | **rot-weiß** | **red-white** |
| ○ | grün-weiß-rot | green-white-red |
| ○ | schwarz-gelb | black-yellow |

> **📝 Explanation:** The state flag of Brandenburg is red-white.

### Question 306 — Civic Education
**🇩🇪** Wo können Sie sich in Brandenburg über politische Themen informieren?
**🇬🇧** Where can you get information about political topics in Brandenburg?

| | Deutsch | English |
|---|---|---|
| ○ | bei den Kirchen | at the churches |
| ○ | bei der Verbraucherzentrale | at the Consumer Advice Centre |
| ✅ | **bei der Landeszentrale für politische Bildung** | **at the State Agency for Civic Education** |
| ○ | beim Ordnungsamt der Gemeinde | at the municipal public order office |

> **📝 Explanation:** Civic/political education information in Brandenburg is provided at the State Agency for Civic Education.

### Question 307 — Capital City
**🇩🇪** Die Landeshauptstadt von Brandenburg heißt …
**🇬🇧** The state capital of Brandenburg is called …

| | Deutsch | English |
|---|---|---|
| ✅ | **Potsdam.** | **Potsdam.** |
| ○ | Cottbus. | Cottbus. |
| ○ | Brandenburg. | Brandenburg (city). |
| ○ | Frankfurt/Oder. | Frankfurt (Oder). |

> **📝 Explanation:** Potsdam. is the state capital of Brandenburg.

### Question 308 — Map Location
**🇩🇪** Welches Bundesland ist Brandenburg?
**🇬🇧** Which (highlighted) federal state is Brandenburg?

![Question 308](../images/308-brandenburg.png)

| | Deutsch | English |
|---|---|---|
| ○ | Bild 1 | Image 1 |
| ○ | Bild 2 | Image 2 |
| ○ | Bild 3 | Image 3 |
| ✅ | **Bild 4** | **Image 4** |

> **📝 Explanation:** Brandenburg surrounds the city-state of Berlin and lies in northeastern Germany, bordering Poland.

### Question 309 — Head of Government
**🇩🇪** Wie nennt man die Regierungschefin/den Regierungschef in Brandenburg?
**🇬🇧** What is the head of government in Brandenburg called?

| | Deutsch | English |
|---|---|---|
| ○ | Erste Ministerin/Erster Minister | First Minister |
| ○ | Premierministerin/Premierminister | Prime Minister |
| ○ | Bürgermeisterin/Bürgermeister | Mayor |
| ✅ | **Ministerpräsidentin/Ministerpräsident** | **Minister-President** |

> **📝 Explanation:** The head of government in Brandenburg holds the title Minister-President.

### Question 310 — Ministries
**🇩🇪** Welche Ministerin/welchen Minister hat Brandenburg nicht?
**🇬🇧** Which minister does Brandenburg NOT have?

| | Deutsch | English |
|---|---|---|
| ○ | Justizministerin/Justizminister | Minister of Justice |
| ✅ | **Außenministerin/Außenminister** | **Minister of Foreign Affairs** |
| ○ | Finanzministerin/Finanzminister | Minister of Finance |
| ○ | Innenministerin/Innenminister | Minister of the Interior |

> **📝 Explanation:** Foreign affairs are a federal responsibility in Germany, not a state matter — so no German state has this ministry.

[⬅ Back to Main README](README.md)