# Bavaria (Bayern) — State Questions (301–310)
[⬅ Back to Main README](README.md)

---

### Question 301 — Coat of Arms
**🇩🇪** Welches Wappen gehört zum Freistaat Bayern?
**🇬🇧** Which coat of arms belongs to Bavaria (Bayern)?

![Question 301](../images/301-bayern.png)

| | Deutsch | English |
|---|---|---|
| ○ | Bild 1 | Image 1 |
| ✅ | **Bild 2** | **Image 2** |
| ○ | Bild 3 | Image 3 |
| ○ | Bild 4 | Image 4 |

> **📝 Explanation:** Bavaria's coat of arms is best known for its white-and-blue diamond pattern (Rautenwappen), representing the historic House of Wittelsbach.

### Question 302 — District
**🇩🇪** Welches ist ein Landkreis in Bayern?
**🇬🇧** Which of these is a district (Landkreis) in Bavaria (Bayern)?

| | Deutsch | English |
|---|---|---|
| ○ | Prignitz | Prignitz |
| ○ | Rhein-Sieg-Kreis | Rhein-Sieg-Kreis |
| ○ | Nordfriesland | North Frisia |
| ✅ | **Altötting** | **Altötting** |

> **📝 Explanation:** Altötting is located in Bavaria (Bayern); the other options are districts in different German states.

### Question 303 — Landtag Term
**🇩🇪** Für wie viele Jahre wird der Landtag in Bayern gewählt?
**🇬🇧** For how many years is the state parliament in Bavaria (Bayern) elected?

| | Deutsch | English |
|---|---|---|
| ○ | 3 | 3 |
| ○ | 4 | 4 |
| ✅ | **5** | **5** |
| ○ | 6 | 6 |

> **📝 Explanation:** The state parliament of Bavaria (Bayern) is elected for a term of 5 years.

### Question 304 — Voting Age
**🇩🇪** Ab welchem Alter darf man in Bayern bei Kommunalwahlen wählen?
**🇬🇧** From what age can people vote in local elections in Bavaria (Bayern)?

| | Deutsch | English |
|---|---|---|
| ○ | 14 | 14 |
| ○ | 16 | 16 |
| ✅ | **18** | **18** |
| ○ | 20 | 20 |

> **📝 Explanation:** In Bavaria (Bayern), the minimum voting age for local elections is 18.

### Question 305 — State Flag
**🇩🇪** Welche Farben hat die Landesflagge von Bayern?
**🇬🇧** What colors does the state flag of Bavaria (Bayern) have?

| | Deutsch | English |
|---|---|---|
| ○ | blau-weiß-rot | blue-white-red |
| ✅ | **weiß-blau** | **white-blue** |
| ○ | grün-weiß-rot | green-white-red |
| ○ | schwarz-gelb | black-yellow |

> **📝 Explanation:** The state flag of Bavaria (Bayern) is white-blue.

### Question 306 — Civic Education
**🇩🇪** Wo können Sie sich in Bayern über politische Themen informieren?
**🇬🇧** Where can you get information about political topics in Bavaria (Bayern)?

| | Deutsch | English |
|---|---|---|
| ○ | beim Ordnungsamt der Gemeinde | at the municipal public order office |
| ✅ | **bei der Landeszentrale für politische Bildung** | **at the State Agency for Civic Education** |
| ○ | bei der Verbraucherzentrale | at the Consumer Advice Centre |
| ○ | bei den Kirchen | at the churches |

> **📝 Explanation:** Civic/political education information in Bavaria (Bayern) is provided at the State Agency for Civic Education.

### Question 307 — Capital City
**🇩🇪** Die Landeshauptstadt von Bayern heißt …
**🇬🇧** The state capital of Bavaria (Bayern) is called …

| | Deutsch | English |
|---|---|---|
| ○ | Ingolstadt. | Ingolstadt. |
| ○ | Regensburg. | Regensburg. |
| ○ | Nürnberg. | Nuremberg. |
| ✅ | **München.** | **Munich.** |

> **📝 Explanation:** Munich. is the state capital of Bavaria (Bayern).

### Question 308 — Map Location
**🇩🇪** Welches Bundesland ist Bayern?
**🇬🇧** Which (highlighted) federal state is Bavaria (Bayern)?

![Question 308](../images/308-bayern.png)

| | Deutsch | English |
|---|---|---|
| ○ | Bild 1 | Image 1 |
| ○ | Bild 2 | Image 2 |
| ○ | Bild 3 | Image 3 |
| ✅ | **Bild 4** | **Image 4** |

> **📝 Explanation:** Bavaria is Germany's largest state by area, located in the southeast, bordering Austria and the Czech Republic as well as several other German states.

### Question 309 — Head of Government
**🇩🇪** Wie nennt man die Regierungschefin/den Regierungschef in Bayern?
**🇬🇧** What is the head of government in Bavaria (Bayern) called?

| | Deutsch | English |
|---|---|---|
| ○ | Erste Ministerin/Erster Minister | First Minister |
| ○ | Premierministerin/Premierminister | Prime Minister |
| ○ | Bürgermeisterin/Bürgermeister | Mayor |
| ✅ | **Ministerpräsidentin/Ministerpräsident** | **Minister-President** |

> **📝 Explanation:** The head of government in Bavaria (Bayern) holds the title Minister-President.

### Question 310 — Ministries
**🇩🇪** Welche Ministerin/welchen Minister hat Bayern nicht?
**🇬🇧** Which minister does Bavaria (Bayern) NOT have?

| | Deutsch | English |
|---|---|---|
| ○ | Justizministerin/Justizminister | Minister of Justice |
| ✅ | **Außenministerin/Außenminister** | **Minister of Foreign Affairs** |
| ○ | Finanzministerin/Finanzminister | Minister of Finance |
| ○ | Innenministerin/Innenminister | Minister of the Interior |

> **📝 Explanation:** Foreign affairs are a federal responsibility in Germany, not a state matter — so no German state has this ministry.

[⬅ Back to Main README](README.md)